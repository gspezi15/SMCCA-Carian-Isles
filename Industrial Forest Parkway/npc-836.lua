...
function sampleNPC.onTickNPC(v)
	...
	if v.stopped > 0 then
		v.stopped = v.stopped - 1
		v.animationTimer = 0
		v.speedX = 0
	elseif v.y == (player.y + player.height - 16.01) then
		v.speedX = (sampleNPCSettings.speed * 2.5) * v.direction
	else
		v.speedX = sampleNPCSettings.speed * v.direction
	end
	
	for k,w in pairs(NPC.get(killprojectiles)) do
