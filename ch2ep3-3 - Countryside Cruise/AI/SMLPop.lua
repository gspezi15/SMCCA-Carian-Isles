...
function pop.onTickEndNPC(v)
	...
	if v.ai2 == 0 then
		...
		local collBox = Colliders.Box(v.x - (v.width * 1), v.y - (v.height * 1), v.width * 1, v.height * 1)
		collBox.x = v.x - v.width * 0
		collBox.y = v.y - v.height  * 0.1
		
		for _,n in ipairs(NPC.get()) do
			if Colliders.collide(n,collBox) then
				if n.idx ~= v.idx and not (NPC.config[n.id].nohurt or n.isHidden or n.friendly or n.isinteractable or n.iscoin) and n:mem(0x138, FIELD_WORD) == 0 then
					playerRiding:harm()
				end
			end
		end
